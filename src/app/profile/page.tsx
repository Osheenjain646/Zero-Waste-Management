...
export default function ProfilePage() {
  ...
  const trustBg = user.trustScore >= 71 ? 'bg-brand-50 text-brand-700 border-brand-200' : user.trustScore >= 41 ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-red-50 text-red-600 border-red-200'
      ...
      <main className="flex-1 overflow-auto">
        <div className="bg-white border-b border-slate-100 px-6 py-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Account details and trust score</p>
          </div>
          {!editing
        ...
        <div className="p-6 max-w-2xl space-y-5">
          ...
          <div className="card p-6">
            <div className="flex items-start gap-4 mb-5">
              <div className="w-16 h-16 bg-brand-100 rounded-2xl flex items-center justify-center text-brand-700 font-bold text-xl shrink-0">
                ...
                {editing ? (
                  ...
                  <>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {user.isVerified && <span className="badge badge-green text-[10px]">Verified ✓</span>}
                    </div>
                    <div className="text-sm text-slate-500">{user.businessName}</div>
                  </>
                )}
                <div className="text-xs text-slate-400 mt-1">{user.email} · {user.role}</div>
              </div>
            </div>
              ...
              {[
                ...
                <div key={item.label}>
                  ...
                  <div className="bg-slate-100 rounded-full h-1.5 overflow-hidden">
                    <div className="h-full bg-brand-300 rounded-full" style={{ width: `${item.val}%` }} />
                  </div>
                </div>
              ))}
            </div>
            <p className="text-xs text-slate-400 mt-4 bg-slate-50 rounded-lg p-3 border border-slate-100">
          ...
          {user.role === 'NGO' && (
            <div className="card p-5">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="font-semibold text-slate-800 mb-1">Subscription</h3>
