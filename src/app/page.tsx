  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 bg-white/90 backdrop-blur border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm text-slate-500">
            <Link href="/map" className="hover:text-brand-600 transition-colors">Live Map</Link>
            <Link href="/leaderboard" className="hover:text-brand-600 transition-colors">Leaderboard</Link>
            <Link href="/prizes" className="hover:text-brand-600 transition-colors">Prize Pool</Link>
          </div>
          <div className="flex items-center gap-2">
            {session ? (
      ...
      <section className="pt-32 pb-24 px-4 relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,#E1F5EE_0%,transparent_60%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_80%,#FAEEDA_0%,transparent_60%)]" />
        <div className="max-w-4xl mx-auto text-center relative">
          <div className="inline-flex items-center gap-2 bg-brand-50 text-brand-700 text-xs font-medium px-3 py-1.5 rounded-full mb-6 border border-brand-100 animate-fade-in">
            <span className="w-1.5 h-1.5 bg-brand-400 rounded-full animate-pulse" />
            Live in Mumbai · Zero food waste pledge
          </div>
          <h1 className="text-5xl md:text-7xl font-bold text-slate-900 leading-tight mb-6 animate-fade-up">
            Feed people.<br />
            <span className="text-brand-400">Waste nothing.</span>
          </h1>
          <p className="text-lg text-slate-500 max-w-xl mx-auto mb-10 animate-fade-up leading-relaxed">
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-xs font-semibold text-brand-600 uppercase tracking-widest">The process</p>
            ...
            {[
              ...
              <div key={i} className="text-center relative z-10">
                <div className="w-14 h-14 rounded-2xl bg-brand-50 border border-brand-100 flex items-center justify-center mx-auto mb-4">
                  <span className="text-brand-600 font-mono text-sm font-bold">{s.n}</span>
                </div>
                <p className="text-xs font-semibold text-brand-500 mb-1 uppercase tracking-wide">{s.role}</p>
                <h3 className="font-semibold text-slate-800 mb-2 text-sm">{s.title}</h3>
      ...
      <section className="py-20 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl font-bold text-slate-900">Everything built in</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              { icon: '◎', color: 'bg-brand-50 text-brand-600', title: 'Live map', desc: 'Real-time Leaflet map shows all active listings. Filter by distance, diet, category.' },
              { icon: '◆', color: 'bg-amber-50 text-amber-600', title: 'Feed Points', desc: 'Earn points for every listing and pickup. Redeem for free subscriptions and perks.' },
              { icon: '◇', color: 'bg-purple-50 text-purple-600', title: 'Prize pool', desc: 'Monthly draw funded by platform activity. Good behaviour earns you entries.' },
              { icon: '◈', color: 'bg-blue-50 text-blue-600', title: 'Trust scores', desc: 'Every user has a live trust score based on ratings, pickups, and cancellations.' },
              { icon: '◉', color: 'bg-teal-50 text-teal-600', title: 'NGO-only buying', desc: 'Only verified organisations can claim. Ensures food reaches communities, not resellers.' },
              { icon: '▦', color: 'bg-rose-50 text-rose-600', title: 'Impact dashboard', desc: 'Track meals saved, CO₂ avoided, streaks, and monthly leaderboard rankings.' },
            ].map((f, i) => (
      ...
      <section className="py-16 px-4 bg-slate-900">
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-xs text-slate-400 uppercase tracking-widest mb-3">Monthly draw</p>
          ...
          <p className="text-slate-400 text-sm mb-8">Funded by cancellation fees, transaction slices, and sponsors. Every completed pickup earns you an entry.</p>
          <Link href="/auth/register" className="inline-block bg-amber-400 text-slate-900 font-semibold px-8 py-3 rounded-xl hover:bg-amber-300 transition-colors">
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-slate-900">NGO plans</h2>
            <p className="text-slate-400 text-sm mt-2">First 60 days free. No credit card required.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              ...
              <div key={p.name} className={`rounded-2xl p-6 border ${p.hot ? 'bg-brand-400 border-brand-400 text-white' : 'bg-white border-slate-100'}`}>
                {p.hot && <div className="text-xs bg-white/20 text-white px-2 py-0.5 rounded-full inline-block mb-3 font-medium">Most popular</div>}
                <div className="font-bold text-xl mb-1">{p.name}</div>
                <div className="text-3xl font-bold mb-1">₹{p.price}<span className={`text-base font-normal ${p.hot ? 'text-white/70' : 'text-slate-400'}`}>/mo</span></div>
                <div className={`my-4 border-t ${p.hot ? 'border-white/20' : 'border-slate-100'}`} />
                ...
                <Link href="/auth/register?role=ngo"
                  className={`block text-center py-2.5 rounded-xl font-medium text-sm transition-all ${p.hot ? 'bg-white text-brand-600 hover:bg-brand-50' : 'bg-brand-50 text-brand-700 hover:bg-brand-100'}`}>
                  Start free trial
                </Link>
              </div>
            ))}
      ...
      <footer className="border-t border-slate-100 py-8 px-4 bg-white">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-700 text-sm">ZeroWaste Marketplace</span>
          </div>
          <p className="text-xs text-slate-400">© 2025 ZeroWaste. Made with purpose in Mumbai, India.</p>
        </div>
      </footer>
    </div>
  )
