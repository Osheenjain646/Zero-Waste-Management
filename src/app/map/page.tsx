    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex items-center justify-between shrink-0">
          <div>
            <h1 className="font-bold text-slate-900 dark:text-white">Live Map</h1>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{filteredListings.length} active listings</p>
          </div>
          <span className="flex items-center gap-1.5 text-xs text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-800/30 px-2.5 py-1 rounded-full border border-brand-100 dark:border-brand-800/50">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-64 shrink-0 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex flex-col overflow-y-auto">
            <div className="p-4 space-y-4 border-b border-slate-100 dark:border-slate-800">
              <div className="text-xs font-medium text-slate-600 dark:text-slate-300 mb-2">Dietary filters</div>
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={vegan} onChange={e => setVegan(e.target.checked)} className="accent-brand-400" />
                  <span className="text-xs text-slate-600 dark:text-slate-300">Vegan only</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={halal} onChange={e => setHalal(e.target.checked)} className="accent-brand-400" />
                  <span className="text-xs text-slate-600 dark:text-slate-300">Halal only</span>
                </label>
              </div>
            ...
            <div className="flex-1 p-3 space-y-2">
              {filteredListings.length === 0 ? (
                <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-8">No active listings</p>
              ) : filteredListings.map(l => (
                <button key={l.id} onClick={() => setSelected(l.id)}
                  className={cn(
                    'w-full text-left p-3 rounded-xl border transition-all',
                    selected === l.id ? 'border-brand-200 dark:border-brand-800/50 bg-brand-50 dark:bg-brand-800/30' : 'border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-slate-200 dark:border-slate-700'
                  )}>
                  <div className="font-medium text-slate-800 dark:text-slate-100 text-xs truncate">{l.foodName}</div>
                  <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{l.quantity} {l.unit}</div>
                  <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 truncate">{l.address?.split(',')[0]}</div>
                </button>
              ))}
            </div>
          </div>
            ...
            {selectedListing && (
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-80 card p-4 shadow-xl z-[1000] animate-fade-up">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{selectedListing.foodName}</h3>
                  <button onClick={() => setSelected(null)} className="text-slate-300 hover:text-slate-500 dark:text-slate-400 text-xl leading-none ml-2">×</button>
                </div>
                <p className="text-xs text-slate-400 dark:text-slate-500 mb-2">📍 {selectedListing.address}</p>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mb-3 flex items-center gap-1 flex-wrap">
                  ...
                  {selectedListing.donor?.isVerified && <VerifiedBadge className="w-3.5 h-3.5 ml-1 text-blue-500" />}
                </p>
                ...
                {!session && (
                  <p className="text-xs text-slate-400 dark:text-slate-500 text-center">
                    <a href="/auth/login" className="text-brand-600 dark:text-brand-400 hover:underline">Log in as NGO</a> to claim
                  </p>
                )}
              </div>
            )}
